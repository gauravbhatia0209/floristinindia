...
const defaultSettings: SiteSettingsData = {
  ...
  defaultMetaDescription: "Premium flower delivery service across India. Same-day delivery available in 100+ cities. Fresh flowers for all occasions with 100% freshness guarantee.",
  defaultOgImage: "",
...
export function useSiteSettings() {
    ...
    try {
      ...
      if (data) {
        ...
        const mappedSettings: SiteSettingsData = {
          ...
          site_tagline: settingsObject.site_tagline || defaultSettings.site_tagline,
          site_description: settingsObject.site_description || defaultSettings.site_description,
          logo_url: settingsObject.logo_url || defaultSettings.logo_url,
          favicon_url: settingsObject.favicon_url || defaultSettings.favicon_url,
          ...
          countryCode: settingsObject.countryCode || defaultSettings.countryCode,
          
          // Operations
          openingHours: settingsObject.openingHours || 
                       settingsObject.business_hours || 
                       defaultSettings.openingHours,
          serviceArea: settingsObject.serviceArea || defaultSettings.serviceArea,
          
          // Social media
          facebook_url: settingsObject.facebook_url || defaultSettings.facebook_url,
          instagram_url: settingsObject.instagram_url || defaultSettings.instagram_url,
          twitter_url: settingsObject.twitter_url || defaultSettings.twitter_url,
          youtube_url: settingsObject.youtube_url || defaultSettings.youtube_url,
        };
  ...
  function generateOrganizationSchema() {
    const schema = {
      ...
      "logo": settings.logo_url ? `${window.location.origin}${settings.logo_url}` : undefined,
      "image": settings.defaultOgImage ? `${window.location.origin}${settings.defaultOgImage}` : undefined,
      "telephone": settings.phone || undefined,
      "email": settings.contact_email || undefined,
      "address": (settings.streetAddress || settings.locality) ? {
        "@type": "PostalAddress",
      ...
      "areaServed": settings.serviceArea ? settings.serviceArea.split(',').map(area => area.trim()) : undefined,
      "sameAs": [
  ...
  function generateMetaTags(pageTitle?: string, pageDescription?: string, pageImage?: string) {
    return {
