...
interface AIOptimizedFAQProps {
  category?: "general" | "delivery" | "products" | "payment" | "corporate";
  limit?: number;
}
...
export default function AIOptimizedFAQ({
  ...
  function getAIOptimizedFAQs(cat: string, lim?: number): FAQ[] {
    const allFAQs: FAQ[] = [
      // General FAQs
      {
      ...
      },
      {
        ...
        answer:
          "We provide flowers for all occasions including birthdays, anniversaries, Valentine's Day, Mother's Day, weddings, corporate events, festivals (Diwali, Christmas, Eid), congratulations, sympathy, get well soon, apologies, housewarming, graduation, and business meetings. Our AI-categorized collection helps you find the perfect flowers for any event or emotion.",
        category: "general",
        keywords: [
          "occasions",
          "birthdays",
      ...
      // Payment FAQs
      {
        ...
        answer:
          "We accept all major payment methods including credit cards (Visa, MasterCard, American Express), debit cards, net banking, UPI (Google Pay, PhonePe, Paytm), digital wallets, and cash on delivery (COD) in select cities. Our secure AI-powered payment system ensures safe transactions with instant confirmation.",
        category: "payment",
        keywords: [
          "payment methods",
          "credit cards",
    ...
    if (cat !== "general") {
      filteredFAQs = allFAQs.filter((faq) => faq.category === cat);
    }
      ...
      {faqs.map((faq) => (
        ...
        >
          <CardContent className="p-6">
            ...
            {expandedFAQ === faq.id && (
              <div
                className="text-muted-foreground leading-relaxed animate-in slide-in-from-top-2 duration-200"
                itemProp="text"
              >
                {faq.answer}
...
export function useAIOptimizedFAQ() {
  const addDynamicFAQ = (
    question: string,
    answer: string,
    category: string,
  ) => {
    console.log("Adding dynamic FAQ for AI:", { question, category });
  };

  const getFAQByKeyword = (keyword: string) => {
