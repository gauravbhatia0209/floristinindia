...
export default function DynamicMetaTags({
  ...
  const { settings, generateMetaTags, generateOrganizationSchema } = useSiteSettings();
  ...
  useEffect(() => {
    ...
    if (metaTags.ogImage) {
      updateMetaTag('og:image', metaTags.ogImage.startsWith('http') ? metaTags.ogImage : `${window.location.origin}${metaTags.ogImage}`, true);
    ...
    if (metaTags.twitterImage) {
      updateMetaTag('twitter:image', metaTags.twitterImage.startsWith('http') ? metaTags.twitterImage : `${window.location.origin}${metaTags.twitterImage}`);
    }
  ...
  }, [title, description, image, type, publishedTime, modifiedTime, location.pathname, settings]);
