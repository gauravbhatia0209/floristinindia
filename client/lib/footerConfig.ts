...
export function getAvailableColumns(maxColumns: number): number[] {
  const max = Math.min(
    Math.max(maxColumns, FOOTER_CONFIG.MIN_COLUMNS),
    FOOTER_CONFIG.MAX_COLUMNS_LIMIT,
  );
...
export function getGridClass(columns: number): string {
  const gridClass =
    FOOTER_CONFIG.GRID_CLASSES[
      columns as keyof typeof FOOTER_CONFIG.GRID_CLASSES
    ];
  return `${FOOTER_CONFIG.RESPONSIVE_CLASSES} ${gridClass || "lg:grid-cols-4"}`;
}

// Helper function to validate column position
export function isValidColumnPosition(
