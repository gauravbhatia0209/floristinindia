      ...
      if (categorySlug) {
        ...
        if (currentCategoryData) {
          ...
          try {
            ...
            const { data: categoryAssignments, error: assignmentsError } =
              await supabase
            ...
            if (categoryAssignments && categoryAssignments.length > 0) {
              ...
              console.log(
                `Found ${productIds.length} products via multi-category assignments`,
              );
            } else {
              // Fallback to legacy category_id approach
              console.log(
                "No multi-category assignments found, using legacy approach",
              );
            ...
            if (productIds.length > 0) {
              ...
              const { data: fullProducts, error: productsError } =
                await supabase
                  .from("products")
                  .select("*")
                  .in("id", productIds)
              ...
              if (productsError) {
                console.error(
                  "Error fetching full product data:",
                  productsError,
                );
                productsData = [];
              } else {
                ...
                console.log(
                  `Fetched ${productsData.length} products for category`,
                );
              }
            } else {
              productsData = [];
            ...
            console.error(
              "Error in multi-category fetch, falling back to legacy:",
              error,
            );
